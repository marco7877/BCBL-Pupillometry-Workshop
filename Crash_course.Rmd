...
affinity for statistical purposes. R is open source and multi-platform
...
So, R is a high order programming language. That means its the language
...
.txt, .csv) where we write with commands for it. RStudio, in turn, is a
nice Graphical User Interface (GUI) that is way way more informative
than looking at plain files because:
...
used like a really powerful calculator that in the future will help us
from writing/solving by hand bothering ANOVAs or Linear models (i.e.,
...
be trivial, but while your coding experience, you are going to use
operations like the aforementioned, thus with **variables**

# Variables

A variable is a data item. Imagine you are an elf with n amount of
magical points. It may be the case that you want to infuze with your MP
some objects. For example, you infuze a sheet with the word *HEAL* with
*healing* .
...
restriction, yo can not use the exactly variable word twice or it may be
overwritten.
...
We have already defined some variables with different data types. Do yo
know which type? If you are like me and have the memory power of Dory
...
elements most of the time. numeric and characters are kinda
...
OK, then in your scroll you have their names and their specie. In this
example Specie would be a factor class. This is because its not a value
...
asking, "Ha, but what if I want many elements inside a variable?" We are
going places here!
...
PhD burn-out, hangover and fantasy stuff like that (blink blink). We'll
then you would use a stone that can store tons of healing spells
...
cool right! \*There'r another multidimentional homogeneous data
...
types inside our list. How cool is that (well, not so much TBH, but its
useful).
...
understand this, imagine DataFrames to be as a Naval Battle board.
...
![install.packages()](./coding.jpg)

R is a programming language used by tons of people. As so, most of the
times (*almost always*) we do not need to reinvent the wheel, because
somebody has already solved our goal and make it available though R
packages. **What are packages** Well, think about it this way: You are
an elf who wants a spell that let you eat as much cookies as you want
...
1 - Install de package (*buy the grimoire*)

2 - Load the package into your working space (*open the grimoire and
have it near hand in case you need it*)
...
Great! we downloaded our first package, but where from? Well, it's
downloaded from The Comprehensive R Archive Network(**CRAN** for R jedys
...
![dark side of the path](./Paths.jpg)


```{r,}
#read a csv with readr::read_csv()
csv.file <- read_csv("~/Documents/BCBL-Pupillometry-Workshop/Exercise.csv")
#read an excel with readxl::read_xlsx
excel.file <- readxl::read_xlsx("~/Documents/BCBL-Pupillometry-Workshop/Exercise_excel.xlsx")

```

If you put attention, you will look that our .csv and our .xlsx are data frames!
This is basically what you are going to do while granted the Jedi knight badge. 
While being a jedi knight you will load your data, sense the R force, make shinny 
...
![I'm one with the force, the force is with me](./Rplots.jpg)
...
![ggplot layers](./ggplot_overview.png)
...
"ehun"))+#what to show on y axis, stops, limits of the axis and labels over breaks
  ...
  size = 0.5, linetype = "solid"),#how is the aspect of the outline, try "dotted"
